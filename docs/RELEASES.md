...
`pyproject.toml` 的 `[project].version` 是 RACS 唯一项目版本号来源。当前项目版本为 `0.1.0`。
...
| --- | --- | --- | --- |
| `v0.1.0` | T-000 | 工程基线 | 候选发布：T-000 已通过 Reviewer，待创建本地标签并完成发布记录 |
| `v0.2.0` | T-001～T-002 | 产品基线 | 未发布：T-002 未完成，且缺少阶段 Reviewer 结论 |
| `v0.3.0` | T-101～T-104 | 基础业务能力 | 规划中 |
| `v0.4.0` | T-201～T-203 | AI 售后流程 | 规划中 |
| `v0.5.0` | T-301～T-304 | 人工协作与恢复 | 规划中 |
