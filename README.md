...
RACS 是一个可恢复式 AI 售后客服项目。阶段一 `v0.2.0`、阶段二 `v0.3.0`、阶段三 `v0.4.0` 和阶段四 `v0.5.0` 已按 Reviewer 门禁发布。项目所有者已验收并批准本地正式 `v1.0.0`：T-401～T-404、T-601～T-608、Docker 发布验证闭环与订单可信依据展示修复均有 Reviewer PASS。该正式版本不推送远程；全部 RC 标签保留不移动。

## 项目版本与发布

项目唯一版本号来源是 `pyproject.toml` 的 `[project].version`，当前正式版本为 `1.0.0`，对应本地 annotated Tag `v1.0.0`；全部 RC 标签保留为历史候选标签。阶段版本规划、发布门禁与一致性检查见 `docs/RELEASES.md`；版本变化见 `docs/CHANGELOG.md`；任务验收证据按 `docs/task-reports/T-xxx.md` 维护。
...
当前正式版本为 `1.0.0` / `v1.0.0`；全部 RC 标签保留不移动。订单查询回复会显示由 Gate 放行的最小依据：订单号、已确认状态和“受控的已授权订单记录”来源；不公开内部 permit、Evidence ID、原始工具参数或推理链。T-608 已获 Reviewer PASS；Docker 复验已覆盖生产 Web 同源 Agent API 代理、四服务健康、关键连通性、Fake 业务路径、日志与清理。历史 `BLOCKED 3/4` 与 `BLOCKED 0/4` 运行仍作为历史事实保留，最终真实 DeepSeek HTTP 4/4 记录以同一工作区 digest 绑定且不包含 Key、推理链或敏感内部数据。正式发布未在本次会话重跑 Docker，详见发布验证记录中的已知限制。
