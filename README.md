...
RACS 是一个可恢复式 AI 售后客服项目。阶段一 `v0.2.0`、阶段二 `v0.3.0` 和阶段三 `v0.4.0` 已按 Reviewer 门禁发布；T-201～T-204 已完成阶段三能力。Agent、审批恢复和产品界面仍属后续范围。

## 项目版本与发布

项目唯一版本号来源是 `pyproject.toml` 的 `[project].version`，当前为 `0.4.0`。阶段版本规划、发布门禁与一致性检查见 `docs/RELEASES.md`；版本变化见 `docs/CHANGELOG.md`；任务验收证据按 `docs/task-reports/T-xxx.md` 维护。
